...
class ESPNCalendarSpider(scrapy.Spider):
    name = 'espncalendar'
    ...
    def parse(self, response):
        ...
        #print(split)
        for day in response.css('section.Card.gameModules'):
            #for day in daily_games.css('h3.Card__Header__Title.Card__Header__Title--no-theme'):
            for scores in day.css('section.Scoreboard.bg-clr-white.flex.flex-auto.justify-between'):
                #try:
                    ...
                    yield{
                        ...
                        'season' : split[9],
                        'week' : split[7],
                        'date' : day.css('h3.Card__Header__Title.Card__Header__Title--no-theme::text').get(),
                        #'awayteam_logo' : scores.css('img.Image.Logo.ScoreboardScoreCell__Logo.mr3.Logo__md::attr(src)').extract()[0],
                        #'awayteam_logo' : scores.xpath('//img/@data-src').extract()[0],
                        'hometeam_logo' : scores.css('img.Image.Logo.ScoreboardScoreCell__Logo.mr3.Logo__md::attr(data-src)').extract()[1],
                        #'hour' : scores.css('div.ScoreCell__Time.ScoreboardScoreCell__Time.h9.clr-gray-03').get(),
                        ...
                        'idgame' : scores.css('section.Scoreboard.bg-clr-white.flex.flex-auto.justify-between::attr(id)').get(),
                        'stadium' : scores.css('div.LocationDetail__Item.LocationDetail__Item--headline::text').get(),
                        'location' : scores.css('div.LocationDetail__Item::text').extract()[1]
...
filename = "espn_2022calendar.json"
...
process.crawl(ESPNCalendarSpider)
process.start()
